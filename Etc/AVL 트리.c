...
TreeNode* restructure(TreeType* T, TreeNode* x) //page 286
{
    ...
    a->right=T1;
    T1->parent=a;
    a->height=max(T0->height, T1->height)+1; //don't forget!!
...
void searchAndFixAfterRemoval(TreeType* T, TreeNode* w)
{
    ...
    //2.------------------
    if(z->left->height > y->right->height)
        y=z->left;
    else
        y=z->right;
    ...
    if(y->left->height > y->right->height)
        ...
        x=y->right;
    else if(z->left==y) //같은 경우 처리
        x=y->left;
    else if(z->right==y) // 같은 경우 처리
        x=y->right;
    ...
    //5. ------------------
    searchAndFixAfterRemoval(T,b); //one more time
}
